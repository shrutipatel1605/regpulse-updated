"use client";

import {
  createContext,
  forwardRef,
  useCallback,
  useContext,
  useMemo,
  useRef,
  useState,
  type ButtonHTMLAttributes,
  type CSSProperties,
  type HTMLAttributes,
  type ReactNode,
  type SVGProps,
} from "react";

export function cn(...parts: Array<string | false | null | undefined>) {
  return parts.filter(Boolean).join(" ");
}

/* ---------------------------------- Pill ---------------------------------- */
export type PillTone = "" | "amber" | "good" | "warn" | "bad" | "blue" | "ghost";
export function Pill({
  children,
  tone = "",
  className,
  style,
  onClick,
}: {
  children: ReactNode;
  tone?: PillTone;
  className?: string;
  style?: CSSProperties;
  onClick?: () => void;
}) {
  return (
    <span className={cn("pill", tone, className)} style={style} onClick={onClick}>
      {children}
    </span>
  );
}

/* ---------------------------------- Tick ---------------------------------- */
export function Tick({
  children,
  right = false,
}: {
  children: ReactNode;
  right?: ReactNode | boolean;
}) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
      <div className="tick">{children}</div>
      {right ? <div style={{ flex: 1, height: 1, background: "var(--line)" }} /> : null}
      {right && typeof right !== "boolean" ? right : null}
    </div>
  );
}

/* ----------------------------------- Btn ---------------------------------- */
type BtnVariant = "" | "primary" | "accent" | "ghost";
type BtnSize = "" | "sm";

export const Btn = forwardRef<
  HTMLButtonElement,
  ButtonHTMLAttributes<HTMLButtonElement> & {
    variant?: BtnVariant;
    size?: BtnSize;
    icon?: boolean;
  }
>(function Btn({ children, variant = "", size = "", icon = false, className, ...rest }, ref) {
  return (
    <button
      ref={ref}
      className={cn("btn", variant, size, icon && "icon", className)}
      {...rest}
    >
      {children}
    </button>
  );
});

/* ---------------------------------- Kbd ----------------------------------- */
export function Kbd({ children }: { children: ReactNode }) {
  return <kbd>{children}</kbd>;
}

/* --------------------------------- Icons ---------------------------------- */
// Hand-rolled minimal line icons — match the prototype 1:1.
const svgBase: SVGProps<SVGSVGElement> = {
  width: 14,
  height: 14,
  viewBox: "0 0 24 24",
  fill: "none",
  stroke: "currentColor",
  strokeWidth: 2,
  strokeLinecap: "round",
  strokeLinejoin: "round",
};

export const Icon = {
  Search: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <circle cx="11" cy="11" r="7" />
      <path d="m21 21-4.3-4.3" />
    </svg>
  ),
  Ask: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <path d="M3 4h18v14h-8l-5 4v-4H3z" />
    </svg>
  ),
  Book: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <path d="M4 4h10a4 4 0 0 1 4 4v12H8a4 4 0 0 0-4 4V4z" />
      <path d="M20 20H8a4 4 0 0 0-4 4" />
    </svg>
  ),
  Pulse: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <path d="M3 12h3l3-9 4 18 3-9h5" />
    </svg>
  ),
  Check: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <path d="M3 12l5 5L21 4" />
    </svg>
  ),
  Flag: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <path d="M4 22V4s3-2 7 0 7 0 7 0v12s-3 2-7 0-7 0-7 0" />
    </svg>
  ),
  Bookmark: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <path d="M5 3h14v18l-7-5-7 5z" />
    </svg>
  ),
  Grid: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <rect x="3" y="3" width="7" height="7" />
      <rect x="14" y="3" width="7" height="7" />
      <rect x="14" y="14" width="7" height="7" />
      <rect x="3" y="14" width="7" height="7" />
    </svg>
  ),
  Spark: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" />
    </svg>
  ),
  Users: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <circle cx="9" cy="8" r="4" />
      <path d="M2 22c0-4 3-7 7-7s7 3 7 7" />
      <circle cx="17" cy="8" r="3" />
      <path d="M22 21c0-3-2-5-5-5" />
    </svg>
  ),
  Settings: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <circle cx="12" cy="12" r="3" />
      <path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z" />
    </svg>
  ),
  Sun: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <circle cx="12" cy="12" r="4" />
      <path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41" />
    </svg>
  ),
  Moon: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8z" />
    </svg>
  ),
  Arrow: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} {...p}>
      <path d="M5 12h14M13 6l6 6-6 6" />
    </svg>
  ),
  Up: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} width={12} height={12} strokeWidth={2.2} {...p}>
      <path d="m6 15 6-6 6 6" />
    </svg>
  ),
  Down: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} width={12} height={12} strokeWidth={2.2} {...p}>
      <path d="m6 9 6 6 6-6" />
    </svg>
  ),
  Tag: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} width={12} height={12} {...p}>
      <path d="M20 12 12 20l-9-9V3h8z" />
      <circle cx="7.5" cy="7.5" r="1" />
    </svg>
  ),
  Plus: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} width={12} height={12} strokeWidth={2.2} {...p}>
      <path d="M12 5v14M5 12h14" />
    </svg>
  ),
  Close: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} width={12} height={12} strokeWidth={2.2} {...p}>
      <path d="M6 6l12 12M18 6 6 18" />
    </svg>
  ),
  Thumb: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} strokeWidth={1.8} {...p}>
      <path d="M7 10v11H3V10zM7 10l5-8a2 2 0 0 1 3 2l-1 6h5a2 2 0 0 1 2 2l-2 8a2 2 0 0 1-2 1H7" />
    </svg>
  ),
  ThumbDown: (p: SVGProps<SVGSVGElement>) => (
    <svg
      {...svgBase}
      strokeWidth={1.8}
      {...p}
      style={{ transform: "scaleY(-1)", ...(p.style || {}) }}
    >
      <path d="M7 10v11H3V10zM7 10l5-8a2 2 0 0 1 3 2l-1 6h5a2 2 0 0 1 2 2l-2 8a2 2 0 0 1-2 1H7" />
    </svg>
  ),
  Pin: (p: SVGProps<SVGSVGElement>) => (
    <svg {...svgBase} width={12} height={12} {...p}>
      <path d="M12 2v8M8 10h8l-1 5H9zM12 15v7" />
    </svg>
  ),
};

/* ------------------------------- Sparkline -------------------------------- */
export function Sparkline({
  data,
  width = 100,
  height = 28,
  stroke = "var(--signal)",
  fill = "rgba(194,90,17,0.12)",
}: {
  data: number[];
  width?: number;
  height?: number;
  stroke?: string;
  fill?: string;
}) {
  if (!data || data.length < 2) return null;
  const max = Math.max(...data);
  const min = Math.min(...data);
  const range = max - min || 1;
  const step = width / (data.length - 1);
  const points = data.map<[number, number]>((v, i) => [
    i * step,
    height - ((v - min) / range) * height,
  ]);
  const path = points
    .map(([x, y], i) => (i === 0 ? `M${x},${y}` : `L${x},${y}`))
    .join(" ");
  const area = `${path} L${width},${height} L0,${height} Z`;
  return (
    <svg width={width} height={height} style={{ display: "block" }}>
      <path d={area} fill={fill} />
      <path d={path} stroke={stroke} strokeWidth={1.25} fill="none" />
    </svg>
  );
}

/* --------------------------------- Avatar --------------------------------- */
export function Avatar({
  initials,
  size = 24,
  tone = "default",
}: {
  initials: string;
  size?: number;
  tone?: "default" | "signal";
}) {
  const bg = tone === "signal" ? "var(--signal)" : "var(--ink-3)";
  const color = tone === "signal" ? "#fff" : "var(--bg)";
  return (
    <div
      style={{
        width: size,
        height: size,
        borderRadius: "50%",
        background: bg,
        color,
        fontFamily: "var(--font-mono)",
        fontSize: size * 0.4,
        fontWeight: 600,
        display: "inline-flex",
        alignItems: "center",
        justifyContent: "center",
        letterSpacing: 0,
        flexShrink: 0,
      }}
    >
      {initials}
    </div>
  );
}

/* ---------------------------------- Toasts -------------------------------- */
export type ToastPayload = { tag?: string; text: string };
type ToastRecord = ToastPayload & { id: string };
type ToastCtx = { push: (t: ToastPayload) => void };
const ToastContext = createContext<ToastCtx | null>(null);

export function ToastProvider({ children }: { children: ReactNode }) {
  const [toasts, setToasts] = useState<ToastRecord[]>([]);
  const timers = useRef<Map<string, ReturnType<typeof setTimeout>>>(new Map());

  const push = useCallback((t: ToastPayload) => {
    const id = Math.random().toString(36).slice(2);
    setToasts((prev) => [...prev, { id, ...t }]);
    const handle = setTimeout(() => {
      setToasts((prev) => prev.filter((x) => x.id !== id));
      timers.current.delete(id);
    }, 3800);
    timers.current.set(id, handle);
  }, []);

  const value = useMemo(() => ({ push }), [push]);

  return (
    <ToastContext.Provider value={value}>
      {children}
      <div className="toast-stack">
        {toasts.map((t) => (
          <div key={t.id} className="toast">
            <span className="tmono">{t.tag || "OK"}</span>
            <span>{t.text}</span>
          </div>
        ))}
      </div>
    </ToastContext.Provider>
  );
}

export function useToast() {
  const ctx = useContext(ToastContext);
  // Soft fallback if a component renders outside the provider (eg SSR shell)
  if (!ctx) return { push: (_: ToastPayload) => undefined };
  return ctx;
}

/* -------------------------- Misc atoms used across ------------------------ */
export function MiniStat({
  label,
  value,
  signal = false,
  tone,
}: {
  label: string;
  value: string | number;
  signal?: boolean;
  tone?: "bad";
}) {
  const color = tone === "bad" ? "var(--bad)" : signal ? "var(--signal)" : "var(--ink)";
  return (
    <div
      style={{
        textAlign: "right",
        borderRight: "1px solid var(--line)",
        paddingRight: 14,
        paddingLeft: 14,
      }}
    >
      <div
        className="tnum"
        style={{
          fontSize: 22,
          fontWeight: 600,
          color,
          letterSpacing: "-0.02em",
          lineHeight: 1,
        }}
      >
        {value}
      </div>
      <div
        className="mono up"
        style={{ fontSize: 9.5, color: "var(--ink-4)", marginTop: 3 }}
      >
        {label}
      </div>
    </div>
  );
}

export function Panel(props: HTMLAttributes<HTMLDivElement>) {
  const { className, ...rest } = props;
  return <div className={cn("panel", className)} {...rest} />;
}
