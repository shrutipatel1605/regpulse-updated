...
class PDFExtractor:
    ...
    @staticmethod
    def is_rbi_access_challenge(pdf_bytes: bytes) -> bool:
        """Detect RBI security/challenge HTML returned instead of a PDF."""
        if not pdf_bytes:
            return False

        preview = pdf_bytes[:100_000].decode("utf-8", errors="ignore").lower()
        ...
        return (
            "<html" in preview
            and any(marker in preview for marker in challenge_markers)
        )
    ...
    async def extract(self, url: str) -> ExtractedDocument:
        ...
        try:
            ...
            if not self.validate_pdf_bytes(pdf_bytes):
                # RBI sometimes returns an HTML security/CAPTCHA page
                # instead of the requested PDF.
                preview = pdf_bytes[:100].decode("utf-8", errors="replace")

                if self.is_rbi_access_challenge(pdf_bytes):
                    logger.warning(
                        "rbi_access_challenge",
                        url=url,
                    ...
                    return ExtractedDocument(
                        raw_text="",
                        extraction_method="rbi_access_challenge",
                        page_count=0,
                        warnings=[
                            "RBI_ACCESS_CHALLENGE: RBI security/CAPTCHA "
                            "page returned instead of PDF."
                        ],
                    )
                ...
                return ExtractedDocument(
                    ...
                    page_count=0,
                    warnings=[
                        f"Not a valid PDF (missing %PDF- header). "
                        f"Got: {preview[:40]}..."
                    ],
                )
